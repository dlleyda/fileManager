	...
	if(nombreValido == TRUE)
	{
		...
		for(i = 0; i < MAX_INODOS; i++)
		{
			...
			if(ext_bytemaps->bmap_inodos[i] == 0)
			{
				...
				strncpy(&ext_bytemaps->bmap_inodos[i], ocupadoS,1);
				inodos->blq_inodos[i].size_fichero = inodos->blq_inodos[inodoPorMirar].size_fichero;
				ext_superblock->s_free_inodes_count--;
				...
				for(j = 0; j < MAX_BLOQUES_PARTICION && contador > 0; j++)
				{
					...
					{
						inodos->blq_inodos[i].i_nbloque[x] = j; //Añadimos ese bloque al inodo
						...
						memcpy(&memdatos->dato[(inodos->blq_inodos[i].i_nbloque[x] - (NUM_BLOQUES-MAX_BLOQUES_DATOS)) * SIZE_BLOQUE], &memdatos->dato[(inodos->blq_inodos[inodoPorMirar].i_nbloque[x] - (NUM_BLOQUES-MAX_BLOQUES_DATOS)) * SIZE_BLOQUE], SIZE_BLOQUE);
				...
				//Almacenamos en contador el inodo que estaba libre para poder ordenar acorde al inodo que ocupa cada fichero
				contador = i;
				
				//Obligamos salirnos del bucle a la primera, ya que siempre vamos a ocupar un solo inodo por fichero
				i = MAX_INODOS;
			}
		}
		//--------------------------------------------------------------------------------------
...
void Grabarinodosydirectorio(EXT_ENTRADA_DIR *directorio, EXT_BLQ_INODOS *inodos, FILE *fich)
{
    // memcpy(&fich,(EXT_SIMPLE_SUPERBLOCK *)&datosfich[0], SIZE_BLOQUE);
...
void GrabarSuperBloque(EXT_SIMPLE_SUPERBLOCK *ext_superblock, FILE *fich)
{
    // memcpy(&fich,, SIZE_BLOQUE);
	printf("GrabarSuperBloque\n");
}
void GrabarDatos(EXT_DATOS *memdatos, FILE *fich)
{
    // memcpy(&fich,&memdatos, SIZE_BLOQUE*MAX_BLOQUES_DATOS);
	printf("GrabarDatos\n");
}
