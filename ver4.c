...
void Directorio(EXT_ENTRADA_DIR *directorio, EXT_BLQ_INODOS *inodos)
{
	...
	for(i = 0; i < MAX_FICHEROS; i++)
	{
		...
		{
			//Hay que darle unas vueltas a lo del nombre
			printf("Nombre: ");
			for(j = 0; j < LEN_NFICH; j++)
			{
				printf("%c", directorio->dir_nfich[i+1346]);
			}
			printf("\t");
			printf("tamaño:%d\tinodo:%d\tbloques:",inodos->blq_inodos[i].size_fichero, directorio->dir_inodo);
