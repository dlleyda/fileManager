	...
	else
	{
		for(i = 0; i < MAX_FICHEROS; i++)
		{
			if(inodos->blq_inodos[i].size_fichero != 0)
			{
				...
				if(comprobarNombres == 0)
				{
					strncpy(&ext_bytemaps->bmap_inodos[i], &unCero,1);
				...
				{
					strncpy(&ext_bytemaps->bmap_bloques[inodos->blq_inodos[i].i_nbloque[j]], &unCero,1);
	...
	if(nombreValido == TRUE)
	{
		...
		for(i = 0; i < MAX_INODOS; i++)
		{
			...
			if(ext_bytemaps->bmap_inodos[i] == 0)
			{
				...
				{
					if(strcmp(&ext_bytemaps->bmap_bloques[j],&unCero) == 0)
					{
						inodos->blq_inodos[i].i_nbloque[x] = j;
						strncpy(&ext_bytemaps->bmap_bloques[j], ocupadoS,1);
						ext_superblock->s_free_blocks_count--;
						
						memcpy(&memdatos->dato[(inodos->blq_inodos[i].i_nbloque[x] - (NUM_BLOQUES-MAX_BLOQUES_DATOS)) * SIZE_BLOQUE], &memdatos->dato[(inodos->blq_inodos[indiceDelQueCopio].i_nbloque[x] - (NUM_BLOQUES-MAX_BLOQUES_DATOS)) * SIZE_BLOQUE], SIZE_BLOQUE);
						x++;
						contador--;
					}
				}
		...
		memcpy(&directorio->dir_nfich[acum], nombredestino, sizeof(nombredestino));
